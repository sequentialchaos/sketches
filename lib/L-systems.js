...
class L_system {
  ...
  copy({axiom=this.axiom, rules=this.rules, len=this.len, angle=this.angle, max_iterations=this.max_iterations} = {}) {
  ...
  draw({x=0, y=0, len=this.len, angle=this.angle, mode='relative', colormode='normal', colors_ratio=1/1.2} = {}) {
    translate(x, y)
    let maxStrokeWeight = map(len, 0, 200, 1, 8)
    strokeWeight(map(this.iterations, 1, this.max_iterations, maxStrokeWeight, 1))
    this.turtle = new Turtle(x, y, len, angle, this.instructions)
    if (colormode == 'normal') {
      stroke(255)
      this.turtle.go({mode:mode})
    } else if (colormode == 'rainbow') {
      this.turtle.goRainbow({mode:mode})
    } else if (colormode == 'rainbow remix') {
      this.turtle.goRainbowRemix({mode:mode, colors_ratio:colors_ratio})
    }
...
class Turtle {
  constructor(x, y, len, angle_delta, instructions) {
    this.x = x
    this.y = y
    this.len = len
    this.angle = 0
  ...
  performInstruction(i, len, angle_delta, mode) {
  ...
  go({x=this.x, y=this.y, len=this.len, angle=this.angle_delta, mode='relative'} = {}) {
    // Go turtle, go! :D
    push()
    if (mode == 'relative') {
      translate(x, y)
    ...
    for (let i of this.instructions) {
      this.performInstruction(i, len, angle, mode)
    }
    pop()
  }

  goRainbow({x=this.x, y=this.y, len=this.len, angle=this.angle_delta, mode='relative'} = {}) {
  ...
  goRainbowRemix({x=this.x, y=this.y, len=this.len, angle=this.angle_delta, colors_ratio=1/7, mode='relative'} = {}) {
    // Go rainbow turtle, go! :D
    push()
    if (mode == 'relative') {
      translate(x, y)
