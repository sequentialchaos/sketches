...
class Layout {
  ...
  remove(hex) {
    if (!this.contains(hex)) {
      ...
      let index = this.hexMap[hex.a][hex.b].index
      this.hexMap[hex.a][hex.b] = null
  ...
  save() {
    let blob = new Blob([layout.toString()], {type: "text/plain;charset=utf-8"});
