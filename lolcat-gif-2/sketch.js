...
function setup() {
  ...
  NUM_EGGS = 10
  cats = []
  // cat = new Emoji(font, emoji, size * 0.88, cat_x, cat_y)
  for (let i = 0; i < NUM_EGGS; i++) {
    multiplier = map(i, 0, NUM_EGGS, 0.92, 0)
    cats.push(new Emoji(font, emoji, size * multiplier, cat_x, cat_y))
    cat_x += 9
  }
  // caption = new Emoji(
  //   font2,
...
function draw() {
  ...
  for (let cat of cats) {
    cat.drawPointsRainbow(20, 10 * animation_frames, 10, frameCount)
  }
  fill(255)
...
class Emoji {
  ...
  drawPointsRainbow(point_diameter, hue_max, hue_offset, frame_count) {
    colorMode(HSB, hue_max - 1, 100, 100, 1)
    for (let i = 0; i < this.points.length; i++) {
      let p = this.points[i]
      let h = map(i, 0, this.points.length - 1, 0, hue_max - 1)
      noStroke()
      fill((h + hue_offset * frame_count) % hue_max, 100, 100, 0.05)
      circle(p.x, p.y, point_diameter)
    }
  }
