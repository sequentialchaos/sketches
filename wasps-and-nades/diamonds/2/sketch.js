...
function draw() {
  ...
  drawShapesRect(drawSquishDiamond, s, s / 2, c.n, normalizedAngle * TWO_PI)
  pop();
...
function drawSquishDiamond(w, h) {
  unitShapeRect(
    w, h,
    [{
        x: 0.5,
...
function unitShapeRect(w, h, vertices) {
  beginShape()
  for (let v of vertices) {
    vertex(w * v.x, h * v.y)
  }
...
function drawShapesRect(f, w, h, n, angle) {
  for (let i = 0; i < n + 8; i++) {
    for (let j = 0; j < n + 8; j++) {
      let x = i * w, //map(i, 0, n, 0, width),
        y = j * h //map(j, 0, n, 0, height);
      push()
